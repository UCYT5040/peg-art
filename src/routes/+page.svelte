<script lang="ts">
	...
	function mouseUp(position: number, e: UIEvent) {
		...
		if (action == 'move') {
			...
			pegHoles[position] = { ...source } as PegConfig;
		}

	}

	function setLibrarySelection(selection: PegConfig) {
		console.log('Set lib select')
...
<div class="flex h-screen w-full">
	<div>
		...
		{#each pegLibrary as row (row)}
			<div class="flex gap-1">
				{#each row as peg (peg)}
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" class="h-12 w-12" role="button" tabindex="0" onmousedown={() => setLibrarySelection(peg)}>
						<Peg {peg} />
					</svg>
				{/each}
			</div>
	...
	>
		{#each pegHoles as peg, index (index)}
			<PegHole {peg} position={index} onmousedown={mouseDown} onmouseup={mouseUp} />
		{/each}
	</div>
</div>
